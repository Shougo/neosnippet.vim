...
function! neosnippet#helpers#vim2json(expr) abort "{{{
  return (exists('*js_encode') && has('patch-7.4.1498')) ? js_encode(a:expr) : string(a:expr)
endfunction "}}}
function! neosnippet#helpers#json2vim(expr) abort "{{{
  sandbox return (exists('*js_decode') && has('patch-7.4.1498')) ? js_decode(a:expr) : eval(a:expr)
endfunction "}}}
